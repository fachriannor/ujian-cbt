...
class M_User extends CI_Model{
 ...
 //        return $this->db->get();
	...
	function q_soal($mapel, $nama_ujian, $jumlah, $kelas){
		//return $this->db->get_where($mapel, $id_siswa);
		$query = 'SELECT * FROM soal WHERE mapel='.$mapel.' AND kelas='.$kelas.'';
		if($nama_ujian == 'Ulangan-1') {
			$query .= ' AND id_kategori = 1 ORDER BY RAND() LIMIT '.$jumlah.'';
		}
		if($nama_ujian == 'Ulangan-2') {
			$query .= ' AND id_kategori = 2 ORDER BY RAND() LIMIT '.$jumlah.'';
		}
		if($nama_ujian == 'Ulangan-3') {
			$query .= ' AND id_kategori =3 ORDER BY RAND() LIMIT '.$jumlah.'';
		}
		if($nama_ujian == 'UTS') {
			$query .= ' AND id_kategori IN(1,2) ORDER BY RAND() LIMIT '.$jumlah.'';	
		}
		if($nama_ujian == 'UAS') {
			$query .= ' AND id_kategori IN(1,2,3) ORDER BY RAND() LIMIT '.$jumlah.'';	
		}
